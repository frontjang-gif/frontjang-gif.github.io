  ...
  class Generator < Jekyll::Generator
    ...
    def folders(site, directory, ancestors)
      Dir.children(directory).sort.filter_map do |name|
        ...
        page_directory = parts.first == "Blog" ? parts.join('/') : "folders/#{parts.join('/')}"
        url = "/" + page_directory.split('/').map { |part| URI.encode_www_form_component(part).gsub("+", "%20") }.join("/") + "/"
        node = { "name" => name, "url" => url, "posts" => posts,
                 "top_level" => ancestors.empty?, "children" => folders(site, path, parts) }
        page = Jekyll::PageWithoutAFile.new(site, site.source, page_directory, "index.html")
