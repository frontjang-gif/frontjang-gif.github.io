...
처음에는 `_config.yml`을 어떻게 구성해야 하는지도 몰랐기 때문에, 기존 테마 예제를 clone해서 시작하기로 했습니다. 여러 테마 중에서는 비교적 깔끔하고 최근까지 업데이트된 Reverie를 선택했습니다.

Reverie 저장소의 `Use this template` 버튼을 누르면 테마를 내 저장소로 복제할 수 있습니다. GitHub Pages용 저장소 이름은 `{사용자명}.github.io` 형식이어야 하므로 `frontjang-gif.github.io`로 만들었습니다. 버튼을 누르고 몇 분이 지나자 배포가 완료되었고, `https://frontjang-gif.github.io`에서 블로그를 확인할 수 있었습니다.
